...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{2.3cm}|X|X|X|}
    ...
    FR-06 & AI Service & Extract thirteen structured fields from classified documents using BIO sequence tagging & Must \\ \hline
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image18.png}
  \caption{BPMN to-be process model showing automated POI validation, CAFF returns, and distribution flows, with residual human touchpoints confined to review, correction, and high-level supervision.}
  \label{fig:image18}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image19.png}
  \caption{FiberGate Ecosystem three-pillar architecture. The Automation Layer handles event-driven and batch workflows; the Intelligence Layer provides multimodal document understanding; the Governance Layer provides the unified operational interface and control plane.}
  \label{fig:image19}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image20.png}
  \caption{Data flow diagram showing all authenticated communication paths across SENTINEL Fleet, FiberGate AI Service, Unified Platform, and external systems. Protocol labels and data direction are annotated on each edge.}
  \label{fig:image20}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image22.png}
  \caption{Azure VNet topology with subnet segmentation (application: 10.0.1.0/24, data: 10.0.2.0/24), NSG allow/deny rules at the boundary, private endpoints for SQL and Blob, auto-scale policy for Spring Boot, and VNet peering to the AI VM.}
  \label{fig:image22}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image23.png}
  \caption{Five-stage pipeline from document upload to CMS Excel output. Stages 1 to 3 (Preprocessing, Classifier, Extractor) are neural; stages 4 and 5 (Completeness Engine, CMS Generator) are deterministic rule engines. The confidence gate at stage 3 determines whether field values are auto-drafted or routed to manual review.}
  \label{fig:image23}
\end{figure}

\subsection{Dataset Strategy and Field Extraction Workflow}

The training corpus was assembled from historical PAR dossiers collected over nine months of operational activity, covering the six document categories present in the LOC PAR workflow: authorisation permit, information sheet, mass plan, situation plan, technical certificate, and mandate. Documents were annotated at the token level using BIO tagging across thirteen field types defined in consultation with the SDM and senior OSA consultants. Dataset imbalance across document classes was addressed through class-weighted loss during fine-tuning.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image24.png}
  \caption{Platform integration architecture showing API contract boundaries between Angular frontend, Spring Boot backend, FastAPI proxy, AI Service, Azure SQL, and the M365 Power Automate layer triggered exclusively by SENTINEL agents.}
  \label{fig:image24}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image25.png}
  \caption{Global use case diagram showing all three actor roles (Consultant, Pilot, SDM) and their interactions across Unified Platform use cases, alongside autonomous SENTINEL agent processes triggered by the Exchange Server.}
  \label{fig:image25}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image26.png}
  \caption{Sequence diagram for the shared email listener and event-driven agent orchestration pattern. HTTP 202 is returned immediately on trigger; the platform polls via job UUID for completion and structured log retrieval.}
  \label{fig:image26}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image27.png}
  \caption{Agent 1 sequence showing Exchange FINOI folder polling, six-field regex extraction with anticipation-row skip logic, Power Automate SUIVI lookup and Planner task creation in the POI alerte bucket.}
  \label{fig:image27}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image28.png}
  \caption{Agent 2 sequence showing \_retour CAFF folder polling, OEIE pattern matching on the email subject line, and atomic Power Automate flow execution combining SUIVI lookup, assignment update, and Planner task creation.}
  \label{fig:image28}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image29.png}
  \caption{Agent 3 sequence showing the six-step pandas processing pipeline, three-sheet workbook generation, Pilot review gate, and per-task webhook dispatch loop with mandatory 2-second inter-call delay.}
  \label{fig:image29}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image30.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image31.png}
  \caption{Unified Platform use case diagram partitioned into four RBAC zones: All Roles (authentication), Consultant, Pilot and SDM, and SDM-only. Each zone is colour-coded to match the actor hierarchy.}
  \label{fig:image31}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image32.png}
  \caption{Sequence diagrams for four representative scenarios: (1) Consultant dossier upload and AI review, (2) Pilot Agent 3 distribution trigger and workbook download, (3) SDM audit trail query and CSV export, and (4) SDM manual agent trigger with live log streaming. Lower panel: functional module summary by role.}
  \label{fig:image32}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image33.png}
  \caption{Domain class diagram covering User, Role, AgentJob, InferenceResult, ExtractedField, AuditEntry, and DistributionRun entities. AuditEntry is insert-only. ExtractedField.correctedValue feeds the AI continuous improvement loop.}
  \label{fig:image33}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image34.png}
  \caption{Component diagram showing Angular, Spring Boot, FastAPI, AI Service, Azure SQL, and Blob Storage with provided/required interfaces. Seven architectural principles are annotated: stateless backend, single integration hub, async polling, inference isolation, private data access, graceful degradation, and independent deployability.}
  \label{fig:image34}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth,max width=\textwidth]{images/image35.png}
  \caption{Deployment diagram showing all FiberGate artefacts within the Azure VNet (App Services, AI VM, SQL, Blob), the GitHub GitHub Actions Azure DevOps CI/CD pipeline, and M365 tenant (Power Automate, Exchange Server) integration boundaries.}
  \label{fig:image35}
\end{figure}
