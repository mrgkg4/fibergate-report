...
\begin{itemize}
  \item Digital information sheets (text-based PDFs) were handled by pdfplumber and regular expressions.
  \item Scanned authorisation permits and mandates were processed by Tesseract 5 (with a French model), regular expressions, and a local Ollama deployment of the qwen2.5:1.5b model.
  \item The three remaining document types went through Tesseract and Ollama for classification only.
\end{itemize}

Testing this prototype revealed three main problems. Each one led to a specific decision in the second version:

\begin{itemize}
  \item \textbf{Regular expressions were fragile.} The pattern used to read the permit reference number had to be rewritten three times during testing, because the format of the documents kept changing. This level of manual work cannot scale to the number of templates seen in production.
  \item \textbf{The small language model was not reliable enough.} The qwen2.5:1.5b model returned invalid JSON in about one mandate call out of six. More importantly, it had no understanding of document layout, so it could not tell apart a target field value from the same text appearing elsewhere on the page. A text-only model was clearly not enough.
  \item \textbf{Per-type branching was costly to maintain.} Every new document type needed its own regular expressions and its own prompts, with nothing shared between branches. The work grew linearly with the number of supported types.
\end{itemize}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.52\textwidth,max width=\textwidth]{images/image56.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.52\textwidth,max width=\textwidth]{images/image57.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.52\textwidth,max width=\textwidth]{images/image59.png}
  \caption{\textit{\textbf{Figure 5.4.} Training data sample: structured JSON representation of an annotated document.}}
  \label{fig:image59}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.52\textwidth,max width=\textwidth]{images/image60.png}
  \caption{\textit{\textbf{Figure 5.5.} Annotated dataset distribution showing the 8.1$\times$ class-imbalance ratio.}}
  \label{fig:image60}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.52\textwidth,max width=\textwidth]{images/image61.png}
  \caption{\textit{\textbf{Figure 5.6.} LayoutLMv3 architecture jointly encoding text, layout, and image patches (adapted from Huang et al., 2022).}}
  \label{fig:image61}
\end{figure}
...
\begin{itemize}
  \item \textbf{Masked Language Modelling (MLM):} 15\% of the text tokens are masked and the model has to reconstruct them from the surrounding context, like in the original BERT.
  \item \textbf{Masked Image Modelling (MIM):} 40\% of the image patches are masked and the model has to predict a discrete visual token for each one, following the BEiT framework.
  \item \textbf{Word-Patch Alignment (WPA):} a binary task in which the model has to decide whether a text token and an image patch are at the same place on the page. This encourages the model to learn how text and image relate to each other.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Document classification head.} A linear layer that maps the 768-dimensional final hidden state of the [CLS] token to six logits, one per document class. A softmax then turns these logits into a probability distribution. The class with the highest probability is the prediction, and that probability is the confidence score used by the gating policy described in Section 5.5.3.
\end{itemize}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.52\textwidth,max width=\textwidth]{images/image62.png}
  \caption{\textit{\textbf{Figure 5.7.} Field-extraction flow converting annotation boxes into per-token BIO labels.}}
  \label{fig:image62}
\end{figure}

\begin{itemize}
  \item \textbf{Stage 1 Reading raw annotations.} Each annotation is exported by Label Studio as a rectangle written in percentage coordinates. These coordinates are converted to pixels using the actual size of the rasterised page. Each record then contains the field type (one of the twelve classes) and the bounding box as the tuple (x\textit{0}, y\textit{0}, x\textit{1}, y\textit{1}).
  \item \textbf{Stage 2 Parallel OCR view.} Tesseract returns a list of OCR tokens, each with its own pixel-level bounding box and a confidence score. The annotations from Stage 1 and the OCR token boxes from Stage 2 are now in the same pixel reference frame, so they can be compared directly.
  \item \textbf{Stage 3 Geometric matching.} For each annotation rectangle, the algorithm goes through every OCR token on the page and computes the intersection-over-union (IoU) between the token box and the annotation rectangle. IoU is the ratio of the overlapping area to the total combined area, and takes values between 0 and 1. A token is assigned to the field whenever its IoU is above 0.30, a value chosen on a held-out subset of pages. This threshold is intentionally low, because OCR boxes tend to be smaller than the rectangles drawn by hand, so a stricter threshold would reject too many valid matches. Among the matched tokens, the first one gets the label B-\{field\}, the next contiguous tokens get I-\{field\}, and all other tokens on the page get O.
  \item \textbf{Stage 4 Subword reconciliation.} The RoBERTa tokeniser used by LayoutLMv3 can split a single OCR word into several sub-tokens. To avoid adding redundant supervision, only the first sub-token of each OCR word receives the BIO label. All continuation sub-tokens, together with the [CLS], [SEP], and padding positions, are assigned the value -100, which tells the cross-entropy loss to ignore them. The final output for each page is a vector of length 512 over 25 label classes plus the -100 mask.
\end{itemize}

The 0.30 IoU cut-off was not assumed but selected empirically on a held-out subset of fifty annotated pages, by sweeping the threshold and measuring the proportion of ground-truth annotation rectangles that received at least one correctly matched OCR token (match yield) against the proportion of matched tokens that fell inside the intended field (match precision). Table~5.8a reports the sweep. Thresholds at or below 0.20 admit spurious tokens from neighbouring fields, while thresholds at or above 0.50 reject hand-drawn rectangles that are systematically larger than the tight OCR boxes; 0.30 maximises yield while keeping match precision above 0.95, and was therefore retained.

\begin{table}[H]
  \centering
  \caption{\textit{\textbf{Table 5.8a.} IoU-threshold sensitivity for annotation-to-token matching (held-out subset of fifty pages).}}
  \renewcommand{\arraystretch}{1.0}
  \footnotesize
  \begin{tabularx}{\textwidth}{|X|X|X|X|}
    \hline
    \textbf{IoU threshold} & \textbf{Match yield} & \textbf{Match precision} & \textbf{Decision} \\ \hline
    0.10 & 0.99 & 0.71 & Too permissive (cross-field leakage) \\ \hline
    0.20 & 0.98 & 0.88 & Permissive \\ \hline
    0.30 & 0.96 & 0.96 & \textbf{Selected (best yield/precision trade-off)} \\ \hline
    0.50 & 0.74 & 0.99 & Too strict (valid spans rejected) \\ \hline
    0.70 & 0.41 & 1.00 & Rejects most hand-drawn boxes \\ \hline
  \end{tabularx}
\end{table}

The resulting token-label distribution is shown in Figure 5.8. The O class represents 95.2\% of training tokens, compared to 1.6\% for B- tokens and 3.2\% for I- tokens. This strong skew justifies the use of inverse-frequency weighting at the token level, mirroring its application at the document level for the classifier.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.52\textwidth,max width=\textwidth]{images/image63.png}
  \caption{\textit{\textbf{Figure 5.8.} Token-label distribution in the extractor training corpus.}}
  \label{fig:image63}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.52\textwidth,max width=\textwidth]{images/image64.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.52\textwidth,max width=\textwidth]{images/image65.png}
...
On the 133-document held-out test set, the best extractor checkpoint (step 301, models/extractor\_v3) reaches a span-level macro-F1 of 0.448 across the twelve fields of the schema. The best F1 observed on the validation set during training was 0.450. Performance varies considerably across fields. Two fields exceed F1 = 0.75 and reach the threshold at which automated extraction can reasonably replace manual verification. Three fields, on the other hand, stay at F1 = 0.000 and need more annotated examples before they can be used in production. The full per-field results, taken directly from the trainer-state log of the deployed checkpoint, are reported in Table 5.10.
...
\begin{itemize}
  \item \textbf{Header dislocation in housing-count extraction.} When the column header falls outside the 512-token window, the model assigns the value to a neighbouring column. The prediction looks confident but is wrong.
  \item Span fragmentation in Disposition\_Mandat\textbf{.} LayoutLMv3 treats line breaks as hard boundaries between spans. A disposition that spans two or three lines is therefore cut into several short pieces instead of being read as one full value.
  \item Spatial ambiguity in Batiment\_Adresse\textbf{.} This field is sometimes predicted as the letterhead address instead of the project address in the body. Both addresses look exactly the same, and only their position on the page tells them apart. When the letterhead is located in a region that looks similar to the project-address region, the model cannot decide reliably.
\end{itemize}
...
The two operating points of this policy (the field-level F1 gate at 0.70 and the per-token confidence gate at 0.60) were both calibrated on the validation partition. Table~5.10a reports the confidence-gate sweep for the fields eligible for automation (F1 $\geq$ 0.70), measuring, at each candidate threshold, the share of predictions accepted without review (automation rate) and the precision of those accepted predictions. A 0.60 gate keeps acceptance precision above 0.97 while still automating roughly four predictions in five; lowering it to 0.50 raises automation but lets through erroneous spans, whereas raising it to 0.70 sacrifices automation for a negligible precision gain. The 0.70 F1 admission threshold was fixed in the same way: it is the lowest field-level F1 at which accepted-prediction precision remained above 0.95 across the validation fields.

\begin{table}[H]
  \centering
  \caption{\textit{\textbf{Table 5.10a.} Per-token confidence-gate sensitivity on automation-eligible fields (validation partition).}}
  \renewcommand{\arraystretch}{1.0}
  \footnotesize
  \begin{tabularx}{\textwidth}{|X|X|X|X|}
    \hline
    \textbf{Confidence gate} & \textbf{Automation rate} & \textbf{Accepted-span precision} & \textbf{Decision} \\ \hline
    0.40 & 0.94 & 0.89 & Too permissive \\ \hline
    0.50 & 0.88 & 0.94 & Permissive \\ \hline
    0.60 & 0.81 & 0.97 & \textbf{Selected} \\ \hline
    0.70 & 0.66 & 0.98 & Over-conservative \\ \hline
    0.80 & 0.43 & 0.99 & Automation collapses \\ \hline
  \end{tabularx}
\end{table}
...
\begin{itemize}
  \item \textbf{Completeness recommendation engine:} applies the LOC PAR business rules and produces a completeness verdict for each demand.
  \item \textbf{CMS pre-fill generator:} writes a 27-column Excel file that follows the operational template. Any field that could not be extracted with enough confidence is flagged for manual completion, using information from the internal Optimum platform of Orange.
\end{itemize}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.52\textwidth,max width=\textwidth]{images/image67.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.52\textwidth,max width=\textwidth]{images/image68.png}
  \caption{\textit{\textbf{Figure 5.14.} CMS pre-fill generation}}
  \label{fig:image68}
\end{figure}
